...
namespace Wbc {
    namespace solver {
        ...
        const QPOutput& QPSolver::solve(const QPData& qp_data) {
            ...
            if (false) {
                ret = solver_->hotstart(qp_data.H.data(), qp_data.g.data(), qp_data.CI.data(), nullptr, nullptr, qp_data.ci_lb.data(),
                                        qp_data.ci_ub.data(), nWSR);
            } else {
                ret = solver_->init(qp_data.H.data(), qp_data.g.data(), qp_data.CI.data(), nullptr, nullptr, qp_data.ci_lb.data(),
                                    qp_data.ci_ub.data(), nWSR);
            }
            ...
            if (ret == qpOASES::SUCCESSFUL_RETURN) {
                ...
                if (ret == qpOASES::RET_MAX_NWSR_REACHED)
                    output.status = QPStatus::MAX_ITER_REACHED;
                if (ret == qpOASES::RET_INIT_FAILED_INFEASIBILITY)
                    output.status = QPStatus::INFEASIBLE;
            }
